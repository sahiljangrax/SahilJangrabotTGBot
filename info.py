...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002106518347').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6874474924').split()]
...
URL = "https://lmmasterbot.onrender.com".format(FQDN) if ON_HEROKU or NO_PORT else \
    "https://lmmasterbot.onrender.com".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://lmmasterbot.onrender.com".format(FQDN)
else:
    URL = "https://lmmasterbot.onrender.com".format(FQDN)
